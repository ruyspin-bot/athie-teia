    ...
    chunks.map(async (ids) => {
      ...
      (data.results || []).forEach((r) => {
        ...
        byFromId[fromId] = (r.to || []).map((t) => {
          // HubSpot returns both the default system type (no label) AND the custom
          // labeled type. Prefer the first type that has a non-empty label.
          ...
          return { toId: t.toObjectId, label };
        });
      });
    })
  );
...
/* ------------------------------------------------------------------ */
async function createAssociation(hs, fromType, fromId, toType, toId, typeId, category) {
