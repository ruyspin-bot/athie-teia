    ...
    chunks.map(async (ids) => {
      ...
      (data.results || []).forEach((r) => {
        ...
        byFromId[fromId] = (r.to || []).map((t) => {
          ...
          // Prefer USER_DEFINED labels (e.g. "Cliente Final", "Broker") over
          // HUBSPOT_DEFINED system labels (e.g. "Primary") — the system label
          // "Primary" was being picked first and blocking role detection.
          const customLabeled = allTypes.find((at) => at.category === 'USER_DEFINED' && at.label && at.label.trim());
          const anyLabeled = allTypes.find((at) => at.label && at.label.trim());
          const label = customLabeled ? customLabeled.label : (anyLabeled ? anyLabeled.label : (allTypes[0] || {}).label || '');
